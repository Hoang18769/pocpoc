...
const useAppStore = create(
  devtools((set, get) => ({
    ...
    navigateToChat: (userId, userInfo = null) => {
      const { conversationMap, chatList } = get();
      const existingChatId = conversationMap.get(userId);
      
      if (existingChatId) {
        // Chat đã tồn tại, điều hướng đến chat với chatId
        console.log(`🚀 Navigating to existing chat: ${existingChatId} with user: ${userId}`);
        return {
          type: 'existing',
