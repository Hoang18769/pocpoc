...
export default function MainLayout({ children }) {
  ...
  const hideRightSidebar =
    pathname.startsWith("/settings") || pathname.startsWith("/search") || pathname.startsWith("/chats");
    ...
    <>
      ...
      <div className="h-screen flex flex-col">
        ...
        <div className="flex flex-1 pt-16 bg-[var(--background)] text-[var(--foreground)] transition-colors duration-500">
          ...
          {/* Main Content */}
          <main className="border flex-1 flex justify-between h-[calc(100vh-64px)] overflow-y-auto px-4">
            <div className="w-full max-w-4xl space-y-6">{children}</div>
          </main>

          {/* Right Sidebar */}
          {!hideRightSidebar && (
           <aside className="border hidden md:flex justify-center items-end w-[80px] lg:w-[400px] lg:max-w-[400px] h-[calc(100vh-64px)] p-4">
