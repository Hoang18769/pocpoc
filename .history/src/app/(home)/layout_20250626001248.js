...
export default function MainLayout({ children }) {
  ...
  // ✅ Đảm bảo userId được set ngay từ đầu
  useEffect(() => {
    const storedUserId = localStorage.getItem("userId");
    const storedToken = localStorage.getItem("accessToken");

    console.log("🔍 [MainLayout] Checking localStorage:", { storedUserId, storedToken });

    if (storedUserId && storedToken) {
      setUserId(storedUserId);
      setToken(storedToken);
      console.log("✅ [MainLayout] userId and token set:", storedUserId);
    } else {
      console.warn("⚠️ [MainLayout] Missing userId or token in localStorage");
    }
  }, []);

  // ✅ Chỉ gọi hooks khi userId đã có giá trị
  const shouldInitializeHooks = userId && token;
  
  // Conditional hook calls - chỉ gọi khi có userId
  const messageNotification = shouldInitializeHooks ? useMessageNotification(userId) : null;
  const notificationSocket = shouldInitializeHooks ? useNotificationSocket(userId, token) : null;
  const onlineNotification = shouldInitializeHooks ? useOnlineNotification(userId) : null;

  // Debug logging
  useEffect(() => {
    console.log("🔍 [MainLayout] Hook status:", {
      userId,
      token: !!token,
  ...
  }, [userId, token, shouldInitializeHooks]);
    ...
    <>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed bottom-4 left-4 bg-black text-white p-2 rounded text-xs max-w-xs">
          ...
          <div>Hooks: {shouldInitializeHooks ? 'Initialized' : 'Waiting'}</div>
          <div>Message Hook: {messageNotification ? 'Active' : 'Inactive'}</div>
        </div>
      )}
    </>
  );
