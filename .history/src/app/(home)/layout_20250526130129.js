  ...
  const layoutContent = (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-1 pt-16 bg-[var(--background)] text-[var(--foreground)] transition-colors duration-500">
        ...
        {/* Sidebar trái */}
        <aside className="w-[80px] border-r h-[calc(100vh-64px)] overflow-y-auto">
          <Sidebar />
        </aside>

        {/* Nội dung chính (căn giữa) */}
        <main className="flex-1 flex justify-center h-[calc(100vh-64px)] overflow-y-auto px-4">
          <div className="w-auto max-w-2xl space-y-6">
            {children}
          </div>
        </main>

        {/* Nút chat bên phải */}
        <aside className="hidden md:flex md:w-[80px] h-[calc(100vh-64px)] items-end justify-center">
