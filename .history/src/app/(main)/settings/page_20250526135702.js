...
export default function SettingsOverview() {
  return (
    <div className=" border w-full flex min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      {/* Sidebar */}
      <aside className="w-[280px] border-r border-[var(--border)] p-4 space-y-4">
        <h2 className="text-sm text-[var(--muted-foreground)] font-semibold">Cách bạn dùng Instagram</h2>
        <nav className="space-y-2">
          {menuItems.map((item, idx) => (
            <button
              key={idx}
              className="w-full flex items-center gap-3 text-left px-3 py-2 rounded-md hover:bg-[var(--muted)] transition-colors"
            >
      ...
      <main className="flex-1 p-8">
        <h1 className="text-2xl font-bold mb-6">Chỉnh sửa trang cá nhân</h1>
        ...
        <div className="bg-[var(--card)] p-6 rounded-lg shadow-md space-y-4">
          <div className="flex items-center gap-4">
            ...
            <button className="ml-auto bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">Đổi ảnh</button>
          </div>
