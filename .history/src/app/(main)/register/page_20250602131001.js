...
useEffect(() => {
  const verifyEmail = async () => {
    ...
    setVerifying(true)
    try {
      const response = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/v1/register/verify`, {
        email: emailParam,
        code: codeParam
      }, {
        headers: {
 ...
 const handleSubmit = async (e) => {
  ...
  if (mode === "register") {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/v1/login`, {
        email,
        password,
      })
