  ...
  return (
    <div className="h-screen bg-[var(--background)] text-[var(--foreground)] flex flex-col">
      ...
      <div className="flex flex-1 pt-16">
        {/* Sidebar - 10% */}
        <div className="p-4 hidden border md:flex sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
          <Sidebar />
        </div>
        <div className="p-4 md:hidden border md:flex sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
          <Sidebar />
        </div>

        {/* Main Content - 50% scrollable */}
        <main className="w-full md:w-[50%] overflow-y-auto h-[calc(100vh-64px)] px-2 sm:px-3 md:px-4 lg:px-6 space-y-6">
          {children}
        </main>
