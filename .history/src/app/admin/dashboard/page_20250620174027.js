...
export default function DashboardPage() {
  ...
  const transformWeeklyData = (data) => {
    ...
    return Object.entries(data).map(([day, value]) => ({
      ...
      value: value === null ? 0 : value, // Convert null to 0 for charts
    }))
  }

  const transformMonthlyData = (data) => {
    if (!data) return []
    // Show all monthly data, not just first 15
  ...
  const transformYearlyData = (data) => {
    if (!data) return []
    // Filter out months with null values
    return Object.entries(data)
      .filter(([month, value]) => value !== null) // Only show months with data
      .map(([month, value]) => ({
