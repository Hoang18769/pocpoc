...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/v1/update-password/verify`, {
          email,
          code,
        })
        setVerified(true)
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/v1/update-password/update`, {
        email,
        code,
