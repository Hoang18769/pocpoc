...
api.interceptors.request.use(
  (config) => {
    try {
      ...
      // Kiểm tra nếu là public endpoint hoặc có flag skipAuth
      if (isPublicEndpoint(config.url) || config.skipAuth) {
        console.log(
          "🔓 Endpoint public hoặc skip auth, bỏ qua token:",
          config.url
        );
        return config;
      }
  ...
  async (error) => {
    ...
    if (!error.response) {
      ...
      if (
        ...
        console.warn(
          "🔄 Network error có thể do token hết hạn, thử refresh..."
        );
    ...
    if (shouldRefresh) {
        ...
        return new Promise((resolve, reject) => {
          addRefreshSubscriber((newToken) => {
            if (newToken) {
              originalRequest.headers["Authorization"] = `Bearer ${newToken}`;
              console.log(
                "🔁 Gửi lại request với token mới sau khi chờ:",
                requestUrl
              );
              resolve(api(originalRequest));
            } else {
              console.error(
                "❌ Không nhận được token mới từ refresh subscriber"
              );
      ...
      try {
        ...
        if (newToken && newToken.length > 0) {
          ...
          throw new Error(
            "❌ Không tìm thấy access token trong phản hồi refresh."
          );
        }
      } catch (refreshError) {
    ...
    // 💡 Handle custom error codes từ backend (nếu có)
    try {
      ...
      const customErrorMessages = {
        ...
        1003: "Sai thông tin đăng nhập.",
        1011: "Token không hợp lệ.",
        1012: "Email đã được sử dụng.",
        1013: "Token hết hạn.",
        1014: "Refresh token không hợp lệ.",
        1015: "Tài khoản chưa xác thực.",
        2003: "Bạn phải đủ 16 tuổi để sử dụng.",
        2012: "Tên người dùng đã tồn tại.",
        4003: "Không thể gửi lời mời kết bạn cho chính mình.",
        4004: "Lời mời đã tồn tại.",
        4005: "Đã là bạn bè.",
        5003: "Bài viết không tồn tại.",
        6000: "Bình luận không tồn tại.",
        7004: "Tin nhắn không tồn tại.",
        9000: "Thiếu từ khóa tìm kiếm.",
      };

      if (errorCode && customErrorMessages[errorCode]) {
        // 🧠 Ưu tiên message riêng từ backend nếu có
        const finalMessage = errorMessage || customErrorMessages[errorCode];
        if (typeof window !== "undefined" && window?.toast !== false) {
          const { toast } = await import("react-hot-toast");
          toast.error(finalMessage);
        }
      }
    } catch (err) {
      console.warn("⚠️ Lỗi khi xử lý mã lỗi tùy chỉnh từ backend:", err);
    }
