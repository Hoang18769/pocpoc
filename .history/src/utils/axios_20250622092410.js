    ...
    if (token) {
      ...
      setTimeout(() => {
        console.log('🔍 Final auth state:', {
          cookieToken: !!cookieUtils.get('token'),
...
api.interceptors.request.use(
  config => {
    ...
    const token = cookieUtils.get('token') || cookieUtils.get('accessToken') || localStorage.getItem("accessToken");
...
// Enhanced setAuthToken function with validation
export function setAuthToken(token, userId, userName) {
  ...
  if (!token || !userId) {
    console.error('❌ Invalid token or userId provided to setAuthToken');
    return false;
  }
...
export function getAuthToken() {
  ...
  return cookieUtils.get('token') || cookieUtils.get('accessToken') || localStorage.getItem("accessToken");
}
...
export function getAuthInfo() {
  ...
  console.log('🔍 Getting auth info:', { 
    ...
    tokenSource: cookieUtils.get('token') ? 'cookie' : localStorage.getItem('token') ? 'localStorage' : 'none',
