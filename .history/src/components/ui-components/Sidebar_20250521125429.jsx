...
export default function SidebarNavigation() {
  ...
  return (
    <div
      className={` border
        fixed  left-0  h-14 w-full
        flex flex-row items-center justify-around

         text-[var(--foreground)] p-2
        md:static md:top-[64px] md:h-[calc(100vh-64px)] 
        md:flex-col md:items-start md:justify-start md:p-4
        
        z-50
      `}
    >
      <nav className=" rounded-xl bg-[var(--card)] flex flex-row md:flex-col items-center justify-around md:justify-start md:space-y-10 w-full p-4">
        {menuItems.map((item) => {
