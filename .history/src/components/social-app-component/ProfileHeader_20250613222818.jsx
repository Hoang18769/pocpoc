...
const handleBlockUser = async () => {
  ...
  const cancelFriendRequest = async () => {
    ...
    try {
      const res=await api.delete(`/v1/friend-request/delete/${uuid}`);
      console.log(res)
  ...
  try {
    const res = await api.post(`/v1/friend-request`);
    if (res.data.code === 200) {
      setFriendRequestStatus("sent");
      // 🔁 Lấy lại uuid ngay sau đó
  ...
  const acceptFriendRequest = async () => {
    ...
    try {
      const res = await api.post(`/v1/friend-request/accept/${uuid}`);
