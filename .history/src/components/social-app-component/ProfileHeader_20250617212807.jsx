...
export default function ProfileHeader({ profileData, isOwnProfile = true, onProfileUpdate }) {
  ...
  const handleChatClick = () => {
    ...
    if (existingChat) {
      console.log("✅ Đã tồn tại chat:", existingChat.id);
      // Nếu đã có, mở chatbox bằng chatId đã tồn tại
      selectChat(existingChat.id);
      return;
    }

    // Nếu chưa có, tạo chat mới thông qua navigateToChat
    ...
    if (chatInfo?.chatId) {
      console.log("✅ Tạo chat mới thành công:", chatInfo.chatId);
      selectChat(chatInfo.chatId);
    } else {
      console.log("❌ Không tạo được chat, mở VirtualChat");
