...
export default function MessageItem({
  ...
  const timeString = dayjs(msg.createdAt).format("HH:mm");
  const tooltipTime = dayjs(msg.createdAt).format("DD/MM/YYYY HH:mm");
  ...
  const renderMediaPreview = (url, fileType) => {
    if (isImageFile(fileType)) {
      return (
        <div onClick={() => handlePreviewClick(url, fileType)} className="cursor-pointer rounded-lg overflow-hidden">
  ...
  const renderFileInfo = (url, fileType, filename, size) => {
    ...
    return (
      <div className="flex items-center gap-2 p-2 bg-[var(--card)] rounded-lg">
        ...
        <a href={url} download={filename} className="p-1 rounded hover:bg-[var(--accent)] text-[var(--foreground)]">
  ...
  return (
    <>
      <div className={clsx("flex items-end gap-2 group message-container", {
        "justify-end": isSelf,
        ...
        <div className={clsx("flex items-end gap-2 max-w-[80%]", {
          ...
          <div className="flex flex-col items-end gap-1 relative">
            ...
            >
              {msg.deleted ? (
                ...
                renderFileInfo(msg.attachedFile.url, msg.attachedFile.contentType, msg.attachedFile.originalFilename || getFilename(msg), msg.attachedFile.size)
              ) : (
                msg.content
              )}

              {msg.edited && !msg.deleted && (
                <div className={clsx("text-xs mt-1 opacity-70", isSelf ? "text-[var(--primary-foreground)]" : "text-[var(--muted-foreground)]")}>
            ...
            {/* Thời gian bên cạnh bong bóng */}
            <div className="text-xs text-muted-foreground opacity-70 select-none" title={tooltipTime}>
              {timeString}
            </div>
