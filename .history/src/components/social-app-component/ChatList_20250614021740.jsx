...
export default function ChatList({ onSelectChat, selectedChatId }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        setIsSearching(true);
        const currentUserId = chats[0]?.target?.userId || ""; // nên thay bằng userId thật
        const res = await api.get(`/v1/chat/search/${currentUserId}?q=${encodeURIComponent(searchTerm)}`);
  ...
  const handleChatSelect = async (chat) => {
    ...
    try {
      if (markChatAsRead) await markChatAsRead(chat.chatId);
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-background flex flex-col border rounded-lg overflow-hidden h-full shadow-sm">
      ...
      {isSearching && (
        <div className="p-2 text-center text-sm italic text-muted-foreground">
          Searching...
        </div>
      )}
