  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        promises.push(
          axios.patch(
            `${process.env.NEXT_PUBLIC_API_URL}/v1/auth/login`users/update-name?givenName=${encodeURIComponent(formData.firstname)}&familyName=${encodeURIComponent(formData.lastname)}`
          ).then(() => {
      ...
      if (formData.birthday !== profileData.birthday) {
        promises.push(
          api.patch(`/v1/users/update-birthdate?birthdate=${encodeURIComponent(formData.birthday)}`)
            .then(() => {
      ...
      if (formData.bio !== profileData.bio) {
        promises.push(
          api.patch(`/v1/users/update-bio?bio=${encodeURIComponent(formData.bio)}`)
            .then(() => {
      ...
      if (avatarFile) {
        ...
        promises.push(
          api.patch("/v1/users/update-profile-picture", form)
            .then(() => {
      ...
      // Gọi callback nếu có cập nhật thành công
      if (updateSuccess) {
        onSave(formData)
      } else {
        console.log("ℹ️ No fields changed, nothing updated.")
      }
    } catch (err) {
