...
export default function EditProfileModal({ profileData, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const compareAndPush = (key, apiField) => {
      if (formData[key] !== (profileData[apiField] || "")) {
        updateFields.push({
          field: key,
          value: formData[key],
          url: `/v1/users/update-${key}`
        })
      }
    }

    compareAndPush("firstname", "givenName")
    compareAndPush("lastname", "familyName")
    compareAndPush("username", "username")
    compareAndPush("birthday", "birthday")
    compareAndPush("bio", "bio")
    if (formData.avatar !== (profileData.profilePictureUrl || "/avatar-placeholder.png")) {
      updateFields.push({
        field: "avatar",
    ...
    try {
      for (const field of updateFields) {
        await api.patch(field.url, { [field.field]: field.value })
      }
