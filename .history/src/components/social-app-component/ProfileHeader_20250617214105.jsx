...
const handleChatClick = () => {
  ...
  // Tìm chat đã tồn tại trong store (giống như ChatList)
  const existingChat = chatList.find(chat => 
    chat.target?.userId === targetUserId || 
    chat.target?.id === targetUserId ||
    chat.target?.username === targetUsername
  );

  if (existingChat) {
    // Nếu có chat tồn tại, đánh dấu đã đọc và chọn chat
    const chatId = existingChat.chatId || existingChat.id;
    selectChat(chatId, existingChat.target);
    
    // Chuyển đến trang chat nếu không ở đó
  ...
  // Nếu chưa có chat, tạo virtual chat
  ...
  // Sử dụng store để hiển thị virtual chat
