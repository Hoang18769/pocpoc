...
export default function ChatBox({ chatId, targetUser, onBack, onChatCreated }) {
  ...
  const [isNewChat, setIsNewChat] = useState(!chatId); // Đánh dấu đây là chat mới
  ...
  // Tạo chat mới khi gửi tin nhắn đầu tiên
  const createNewChat = async (message) => {
    try {
      const response = await api.post('/v1/chat/create', {
        receiverUsername: targetUser?.username,
        message: message
      });
      ...
      if (response.data?.chatId) {
        ...
        // Callback để parent component cập nhật danh sách chat
        if (onChatCreated) {
