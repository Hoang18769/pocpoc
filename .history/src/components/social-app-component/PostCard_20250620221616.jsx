...
export default function PostCard({ post, liked, onLikeToggle, size = "default", className = "" }) {
  ...
  // Determine which post data to use for the modal and interactions
  ...
  return (
    ...
    >
      ...
      <p className={textSizes.likes}>
        {post.sharedPost ? post.originalPost?.likeCount || 0 : post.likeCount} lượt thích
      </p>

      {(post.sharedPost ? post.originalPost?.latestComment : post.latestComment) && (
        <div className={textSizes.comment}>
          <span className="font-semibold">
            {post.sharedPost ? post.originalPost.latestComment?.user : post.latestComment?.user}
          </span>
          <span className="ml-2">
            {post.sharedPost ? post.originalPost.latestComment?.content : post.latestComment?.content}
          </span>
        </div>
      )}
      ...
      >
        Xem tất cả {post.sharedPost ? post.originalPost?.commentCount || 0 : post.commentCount} bình luận
      </button>

      {isModalOpen && (
        <PostModal
          post={getPostForModal()}
          liked={post.sharedPost ? post.originalPost?.liked : liked}
          likeCount={post.sharedPost ? post.originalPost?.likeCount : post.likeCount}
