...
export default function UserPreviewResponsive({
  user,
  variant = "post",
  lastonline = true,
  isme = false,
  size = "default", // Có thể là "compact", "default", hoặc "large"
  className,
}) {
  ...
  const getIconSize = () => {
    if (isMobile) {
      return size === "compact" ? 14 : size === "large" ? 16 : 15
    }
  ...
  return (
    <div
      className={`
        ...
        ${size === "compact" ? "p-1.5" : size === "large" ? "p-3" : "p-2"}
        ${className || ""}
      `}
    >
      {/* Avatar */}
      ...
      <div className={`ml-2 sm:ml-3 flex-grow min-w-0`}>
        <h4
          ...
          ${size === "compact" ? "text-xs" : size === "large" ? "text-sm" : "text-xs sm:text-sm"}
        `}
        >
          {userData.name}
        </h4>
        ...
        {lastonline && (
          <p
            ...
            ${size === "compact" ? "text-[10px]" : size === "large" ? "text-xs" : "text-[10px] sm:text-xs"}
          `}
          >
            {userData.lastOnline}
          </p>
      ...
      <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        ...
        {variant === "post" && isme && (
          <button
            ...
            ${size === "compact" ? "p-1" : size === "large" ? "p-2" : "p-1.5"}
          `}
          >
