...
export default function PostModal({
  ...
  return (
    <Modal isOpen={true} onClose={onClose}>
      ...
      {/* Image side */}
<div
  className="relative w-full md:w-3/5 bg-black overflow-hidden touch-pan-x"
...
>
  <div className="absolute inset-0 z-10 flex items-center justify-between px-2 pointer-events-none">
    {page.index > 0 && (
      <button
        onClick={showPrev}
        className="pointer-events-auto p-1 bg-black/50 hover:bg-black/70 text-white rounded-full"
      >
        <ChevronLeft />
      </button>
    )}
  ...
  <motion.div
    ...
    animate={{ x: `-${page.index * (100 / images.length)}%` }}
