...
export default function EditProfileModal({ profileData, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Cập nhật name nếu có thay đổi
      if (
      ...
      if (formData.username !== profileData.username) {
        try {
          await api.patch(
            ...
            new URLSearchParams({ username: formData.username }),
            { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
          )
      ...
      if (formData.birthday !== profileData.birthday) {
        try {
          await api.patch(
            "/v1/users/update-birthdate",
            new URLSearchParams({ birthdate: formData.birthday }),
            { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
          )
      ...
      if (formData.bio !== profileData.bio) {
        try {
          await api.patch(
            "/v1/users/update-bio",
            new URLSearchParams({ bio: formData.bio }),
            { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
          )
      ...
      // 5. Cập nhật avatar nếu có file mới
      if (avatarFile) {
        try {
      ...
      onSave(formData) // Gửi dữ liệu mới về ProfileHeader
    } catch (err) {
