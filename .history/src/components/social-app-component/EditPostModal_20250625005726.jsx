...
export default function EditPostModal({ isOpen, onClose, post, onPostUpdated }) {
  ...
  useEffect(() => {
    if (post) {
      ...
      // Chuyển file dạng string thành object preview
      setMedia(
        (post.files || []).map((url) => ({
          id: null, // không cần id ở đây
          file: null,
          preview: url,
  ...
  const handleRemoveMedia = (index) => {
    ...
    // Nếu file cũ thì ghi nhận id để xóa
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      media.filter((m) => m.file).forEach((m) => formData.append("files", m.file));
      ...
      if (res.data.code === 200) {
        ...
        onPostUpdated?.({
          ...
          privacy: newPrivacy,
          files: media
            .filter((m) => !removedFileIds.includes(m.id)) // chỉ lấy file không bị xóa
            .map((m) => m.preview), // trả về danh sách preview/url
        });
        onClose();
      } else {
