...
export default function ChatItem({ chat, onClick, selected }) {
  ...
  const getLastOnlineText = () => {
    ...
    if (lastOnline) {
      const lastOnlineTime = dayjs(lastOnline);
      const now = dayjs();
      const diffHours = now.diff(lastOnlineTime, 'hour');
      ...
      <div className="flex-1 min-w-0 flex flex-col hide-between-630-768">
        ...
        <div className="flex justify-between items-center">
          <div className="flex flex-col min-w-0 flex-1">
            ...
            {!isOnline && (
              <p className="text-xs text-muted-foreground/70 truncate hidden md:block">
                {lastOnlineText}
              </p>
            )}
          </div>
