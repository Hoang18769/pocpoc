...
import InputInlineLabel from "../ui-components/Input";
import { Button } from "@/components/ui/button";
import { Avatar } from "@/components/ui-components/Avatar";

export default function Chatbox() {
  const [messages, setMessages] = useState([
    { type: "system", text: "Hi Sally! Welcome to Name. Thanks for getting in touch with us on Messenger. Can we assist with lorem ipsum?" },
  ...
  return (
    <div className="w-full max-w-md mx-auto shadow-lg rounded-2xl border flex flex-col h-[600px]">
      {/* Header */}
      <div className="flex items-center gap-3 p-4 border-b">
        <Avatar src="/assests/photo/AfroAvatar.png" alt="User" />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50 text-sm">
        {messages.map((msg, idx) => (
          ...
          >
            {msg.type === "bot" ? (
              <>
                <Avatar src="/assests/photo/AfroAvatar.png" size="xs" />
                <div>{msg.text}</div>
              </>
            ) : (
              msg.text
