...
export default function ChatList({ onSelectChat, selectedChatId }) {
  ...
  // Initialize unread counts from store data
  useEffect(() => {
    const counts = {};
  ...
  const handleChatSelect = async (chat) => {
    ...
    // Mark as read in backend and store
    try {
      // Use store method if available, otherwise direct API call
      if (markChatAsRead) {
        await markChatAsRead(chat.chatId);
      } else {
        await api.put(`/v1/chats/${chat.chatId}/read`);
      }
    } catch (err) {
    ...
    // Notify parent component - truyền đúng như bản API
    onSelectChat(chat.chatId, chat.target);
  };

  // Debounced search API call - giữ nguyên timeout 3000ms như bản API
  useEffect(() => {
    if (!searchTerm) {
    ...
    }, 3000); // Giữ nguyên 3000ms như bản API
  ...
  const uniqueChats = [
    ...new Map(chatList.map(chat => [chat.target.userId, chat])).values(),
  ];

  // Loading state - giống như bản API
