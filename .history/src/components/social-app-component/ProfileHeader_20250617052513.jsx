...
import useAppStore from "@/store/ZustandStore"; // Import store
...
export default function ProfileHeader({ profileData, isOwnProfile = true, onProfileUpdate }) {
  ...
  // Sử dụng store để xử lý chat navigation
  ...
  const handleChatClick = () => {
    // Cần có userId của người được xem profile
    // Giả sử profileData có userId hoặc id
    const targetUserId = profileData.id || profileData.userId;
    ...
    // Sử dụng store để xử lý navigation
    const chatInfo = navigateToChat(targetUserId, {
      username: profileData.username,
      fullName: `${profileData.givenName || ""} ${profileData.familyName || ""}`.trim(),
      avatar: profileData.profilePictureUrl
    });

    // Điều hướng về trang chats chính và để store handle việc hiển thị chat
    router.push('/chats');
    
    // Set timeout để đảm bảo navigation hoàn tất trước khi trigger chat selection
    setTimeout(() => {
      if (chatInfo.type === 'existing') {
        // Trigger store để select existing chat
        useAppStore.getState().selectChat(chatInfo.chatId);
      } else {
        // Trigger store để show virtual chat
        useAppStore.getState().showVirtualChat(targetUserId, {
          username: profileData.username,
          fullName: `${profileData.givenName || ""} ${profileData.familyName || ""}`.trim(),
          avatar: profileData.profilePictureUrl
        });
