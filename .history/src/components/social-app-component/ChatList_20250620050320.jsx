        ...
        {filteredChats.length > 0 ? (
          filteredChats.map((chat) => (
            <div key={chat.chatId || chat.id}>
              ...
              />
              {/* ✅ Debug info - chỉ hiển thị cho chat được chọn và có development mode */}
              {process.env.NODE_ENV === 'development' && selectedChatId === (chat.chatId || chat.id) && (
                <div className="px-4 py-1 text-xs text-blue-600 bg-blue-50 border-b">
                  🔍 Debug: Count = {chat.notReadMessageCount}, ID = {chat.chatId || chat.id}
                </div>
              )}
            </div>
          ))
