...
export default function PostCard({ post, liked, onLikeToggle, size = "default", className = "" }) {
  ...
  useEffect(() => {
    ...
    console.log("PostCard passing images to ImageView:", post.images)
  ...
  // ✅ Map post.files (string[]) to { src }[]
  const imageUrls = Array.isArray(post.files) ? post.files.map((url) => ({ src: url })) : []
  ...
  return (
    <Card className={`flex flex-col ${padding} w-full ${className}`}>
      ...
      {/* ✅ Image grid hiển thị đúng */}
      {imageUrls.length > 0 && (
<ImageView images={post.files.map(f => typeof f === 'string' ? f : f.src)}
