...
export default function ProfileHeader({ profileData, isOwnProfile = true, onProfileUpdate }) {
  ...
  const cancelFriendRequest = async () => {
    try {
      const res = await api.delete(`/v1/friend-request/delete/${profileData.username}`);
      console.log(res);
  ...
  const declineFriendRequest = async () => {
    try {
      const res = await api.delete(`/v1/friend-request/delete/${profileData.request?.id}`);
      console.log(res);
  ...
  const sendFriendRequest = async () => {
    try {
      const res = await api.post(`/v1/friend-request/send/${profileData.username}`);
      if (res.data.code === 200) {
        onProfileUpdate({
          ...profileData,
  ...
  const acceptFriendRequest = async () => {
    try {
      const res = await api.post(`/v1/friend-request/accept/${profileData.request?.id}`);
      if (res.data.code === 200) {
  ...
  const unfriend = async () => {
    try {
      const res = await api.delete(`/v1/friends/${profileData.friendId}`);
      console.log(res);
  ...
  const renderFriendButton = () => {
    ...
    if (profileData.request) {
      if (profileData.request.direction === "OUT") {
        return (
          <button
      ...
      } else if (profileData.request.direction === "IN") {
        return (
