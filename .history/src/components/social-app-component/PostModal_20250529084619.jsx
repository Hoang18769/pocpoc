...
export default function PostModal({ post, activeIndex, onClose }) {
  ...
  const showNext = () => {
    if (index < post.images.length - 1) setIndex(index + 1)
  }
  ...
  return (
    <Modal isOpen={true} onClose={onClose}>
      <div className="flex flex-col md:flex-row w-full h-[80vh] bg-white dark:bg-neutral-900 rounded-xl overflow-hidden">
        {/* Ảnh bên trái */}
        <div
        ...
        <div className="w-full md:w-2/5 p-4 flex flex-col justify-between overflow-y-auto">
          <div className="overflow-y-auto flex-1 mb-4">
            ...
            <div className="flex gap-4 mb-2 text-[color:var(--muted-foreground)] text-lg">
              <button><Heart className="h-5 w-5" /></button>
              <button>💬</button>
              <button>📤</button>
            </div>

            <p className="text-xs text-muted mb-2">{post.likes} likes</p>
