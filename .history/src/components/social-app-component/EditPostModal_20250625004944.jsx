...
export default function EditPostModal({ isOpen, onClose, post, onPostUpdated }) {
  ...
  const handleFileInputAdd = (files) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length > 0) handleFileInputAdd(files);
    e.target.value = null;
  };
        ...
        <div className="mt-4">
          <ImagePreview
            ...
            onImageClick={(index) => {
              ...
              if (src) window.open(src, "_blank"); // hoặc bạn set state để phóng to trong Modal riêng
            }}
          />
          <input
            type="file"
