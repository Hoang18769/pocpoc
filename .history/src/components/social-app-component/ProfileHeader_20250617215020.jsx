...
const handleChatClick = () => {
  ...
  console.log("🔍 handleChatClick triggered:", { targetUserId, targetUsername });
  ...
  // Tìm chat đã tồn tại trong store
  const existingChat = chatList.find(chat => {
    console.log("🔍 Checking chat:", chat);
    return chat.target?.userId === targetUserId || 
  ...
  if (existingChat) {
    // Chat tồn tại
    const chatId = existingChat.chatId || existingChat.id;
    console.log("✅ Using existing chat:", chatId);
    
    markChatAsRead(chatId);
    selectChat(chatId, existingChat.target);
  ...
  // Debug: Kiểm tra store sau khi update
  setTimeout(() => {
    console.log("🔍 Store after update:", {
      selectedChat: useAppStore.getState().selectedChat,
      virtualChat: useAppStore.getState().virtualChat
    });
  }, 100);
