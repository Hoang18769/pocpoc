...
export default function PostCard({ post, size = "default", className = "" }) {
  ...
  const getImageSize = () => {
    if (isMobile) {
      ...
      return { width: size === "compact" ? "200px" : size === "large" ? "300px" : "200px", height: "auto" }
    }
  }
  ...
  return (
    <Card className={` ${layout} ${padding} ${className}`}>
      {/* LEFT SIDE (Text) */}
      ...
      {post.image && (
        <div
          className={`rounded-lg overflow-hidden ${isMobile ? "w-full mt-3" : "ml-4"}`}
          style={{
