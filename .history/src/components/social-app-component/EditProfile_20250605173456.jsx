...
export default function EditProfileModal({ profileData, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Test hardcode bio update
    if (formData.bio !== profileData.bio) {
      try {
        console.log('Updating bio:', formData.bio)
        const response = await api.patch(`/v1/users/update-bio?bio=${encodeURIComponent(formData.bio)}`)
        console.log('Bio update response:', response)
        setSuccessMessages(['Bio updated successfully'])
      } catch (err) {
        console.error('Bio update error:', err)
        console.error('Error response:', err.response)
        setErrors({ bio: err.response?.data?.message || 'Failed to update bio' })
      }
    }

    setLoading(false)
    return // Tạm thời return để chỉ test bio
    ...
    const updates = [
      ...
      },
      {
        ...
        request: () => {
          // Thử cách 1: Query parameter như Postman
          const url = `/v1/users/update-bio?bio=${encodeURIComponent(formData.bio)}`
          console.log('Bio update URL:', url)
          console.log('Bio value:', formData.bio)
          
          // Ghi đè Content-Type cho request này
