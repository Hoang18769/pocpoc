...
export default function PostCard({ post, liked, onLikeToggle, size = "default", className = "" }) {
  ...
  try {
    ...
    if (newContent !== post.content) {
      requests.push(
        api.patch("/v1/posts/update-content", null, {
          params: { id: post.id, content: newContent }
        })
      )
    }
    if (newPrivacy !== post.privacy) {
      requests.push(
        api.patch("/v1/posts/update-privacy", null, {
          params: { postid: post.id, privacy: newPrivacy }
        })
      )
    }
