...
export default function ChatList({ onSelectChat, selectedChatId }) {
  ...
  if (loading) {
    return <div className="space-y-3 p-4">...</div>; // Giữ nguyên loading state
  }

  if (error) {
    return <div className="...">...</div>; // Giữ nguyên error state
  }

  if (chats.length === 0) {
    return <div className="...">...</div>; // Giữ nguyên empty state
  }
  ...
  if (!expanded && !isChatsPage) {
    return (
      <div 
        className="flex items-center justify-between p-2 bg-background border rounded-lg cursor-pointer hover:bg-accent"
  ...
  // Expanded mode - hiển thị từ dưới lên
  return (
    <div className="flex flex-col border rounded-lg overflow-hidden h-full">
      ...
      {/* Danh sách chat - hiển thị từ dưới lên */}
      <div 
        ref={listRef}
