...
export default function ProfileHeader({ profileData, isOwnProfile = true, onProfileUpdate }) {
  ...
  const handleChatClick = () => {
    ...
    const existingChatId = store.getChatByUserId(targetUserId);

    router.push("/chats");

    setTimeout(() => {
      if (existingChatId) {
        store.selectChat(existingChatId);
        console.log("🔁 Mở lại chat:", existingChatId);
      } else {
        store.showVirtualChat(targetUserId, {
          username: profileData.username,
          displayName: `${profileData.givenName || ""} ${profileData.familyName || ""}`.trim(),
          avatar: profileData.profilePictureUrl,
        });
        console.log("🆕 Tạo virtual chat:", targetUserId);
      }
    }, 100);
  };
