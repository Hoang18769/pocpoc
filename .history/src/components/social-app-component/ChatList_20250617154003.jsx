...
export default function ChatList({ onSelectChat, selectedChatId }) {
  ...
  // Fetch chat list on mount if empty
  useEffect(() => {
  ...
  // Scroll to bottom on chatList update
  useEffect(() => {
    if (listRef.current) {
  ...
  }, [searchTerm, searchChats, clearSearch]);
  ...
  const handleChatSelect = async (chat) => {
    ...
    useChatStore.setState(state => ({
      unreadCounts: { ...state.unreadCounts, [chat.chatId]: 0 }
    }));
    ...
    try {
      ...
      // Rollback UI if needed handled inside store
    }
        ...
        {filteredChats.length > 0 ? (
          ...
          <div className="p-4 text-center text-muted-foreground text-sm">
            {isSearching ? "Searching..." : 
             searchTerm ? "No matches found" : "Loading..."}
          </div>
        )}
      </div>
    </div>
