...
export default function ProfileHeader({ profileData, isOwnProfile = true, onProfileUpdate }) {
  ...
  const handleChatClick = () => {
    ...
    // Tìm xem liệu có chat nào với người này đã tồn tại chưa
    const existingChat = chatList.find(
      (chat) => chat.participants?.some((p) => p.username === username)
    );

    if (existingChat) {
      console.log("✅ Đã chọn chat:", existingChat.id);
      // Nếu tồn tại thì chọn chat đó
      selectChat(existingChat.id);
      return;
    }

    // Nếu không, cố gắng khởi tạo cuộc trò chuyện mới
    ...
    if (chatInfo?.chatId) {
      console.log("✅ Đã tạo chat mới:", chatInfo.chatId);
      selectChat(chatInfo.chatId);
    } else {
      console.log("❌ Không tìm thấy chat, mở VirtualChat");
