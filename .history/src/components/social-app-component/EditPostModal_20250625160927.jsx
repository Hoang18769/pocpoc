  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (newPrivacy !== post.privacy) {
        const privacyRes = await api.patch(`/v1/posts/update-privacy/${post.id}`, {
          privacy: newPrivacy
        });
      ...
      // 2. Update content nếu có thay đổi
      if (newContent !== post.content) {
        const contentRes = await api.patch(`/v1/posts/update-content/${post.id}`, {
          content: newContent
        });
      ...
      // 3. Update files nếu có thay đổi (chỉ cho non-shared posts)
      if (canEditFiles && (removedFileIds.length > 0 || media.some(m => !m.isOld))) {
        ...
        const filesRes = await api.put(`/v1/posts/${post.id}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        
        if (filesRes.data.code !== 200) {
          throw new Error(filesRes.data.message || "Lỗi khi cập nhật files!");
        }
        
        // Cập nhật files
        const existingFiles = media.filter((m) => m.isOld && !removedFileIds.includes(m.id)).map((m) => m.preview);
        const newFiles = filesRes.data.body?.files || [];
