...
export default function PostCard({ user, timestamp, content, image, likes, comments }) {
  ...
  const formatLikes = (num) => (num > 999 ? `${(num / 1000).toFixed(1)}k` : num.toString())
  ...
  return (
    <div className="bg-gray-100 dark:bg-[var(--card)] rounded-xl p-4 max-w-3xl mx-auto shadow-sm">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <button className="p-1" aria-label="Options">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className={`${image ? "md:w-1/2" : "w-full"}`}>
          ...
          <div className="flex items-center gap-2 mt-2">
            <button className="p-1" onClick={() => setLiked(!liked)} aria-label="Like">
            ...
            <button className="p-1 ml-auto" onClick={() => setSaved(!saved)} aria-label="Save">
              <Bookmark className={`w-5 h-5 ${isClient && saved ? "fill-gray-600 text-gray-600" : "text-gray-600 dark:text-gray-300"}`} />
            </button>
          </div>

          {isClient && <p className="text-sm font-medium mt-2">{formatLikes(likes)} likes</p>}
        </div>
        ...
        {image ? (
          <div className="md:w-1/2 relative bg-pink-200 dark:bg-gray-800 rounded-lg overflow-hidden">
            <div className="aspect-square relative">
              ...
              <button className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/70 p-1 rounded-full" aria-label="Previous image">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/70 p-1 rounded-full" aria-label="Next image">
            ...
            <div key={index} className="flex items-start gap-2 mb-2">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <button className="p-1" aria-label="More comment options">
              ...
              <button className="p-1" aria-label="Like comment">
