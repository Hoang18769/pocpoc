      ...
      client.subscribe(`/chat/${chatId}`, (message) => {
        try {
          ...
          setMessages((prev) => {
            ...
            // Cập nhật chatList với tin nhắn mới nhất (là tin nhắn đầu tiên trong mảng)
            const { chatList } = useAppStore.getState();
            console.log("📜 chatList trước khi update:", chatList);
            console.log("datachat", data);
            
            const foundChat = chatList.find((c) => c.chatId === data.chatId);

            if (foundChat) {
              // Sử dụng tin nhắn mới nhất (đầu tiên trong mảng sau khi thêm)
              const latestMessage = updatedMessages[0];
              
              const updatedChat = {
                ...foundChat,
                lastMessage: latestMessage, // Sử dụng tin nhắn mới nhất
              ...
              console.log("📜 chatList sau khi update:", newChatList);
            } else {
